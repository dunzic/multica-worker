...
- Human/local profile and daemon commands — including `login`, `logout`, `setup`, `workspace switch`, local runtime profile path mutation, `daemon start` / `stop` / `restart`, `daemon logs`, `daemon probe-runtimes`, and every `daemon role-source` configuration command — are unavailable. `daemon stop` in particular would terminate the daemon running this task and every sibling task on it; role-source configuration would otherwise expose or mutate Owner-controlled local source authority.
