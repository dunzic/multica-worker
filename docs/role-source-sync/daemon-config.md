...
- `multica_signed_remote` publishes the same contract through a strict signed HTTPS envelope. Its configuration contains credential-free same-origin bundle/artifact URLs, an issuer and one to three named Ed25519 public keys; remote-only documents may leave `allowed_roots` empty. Redirects, proxies, IP literals, private DNS results, non-default ports, unknown signing keys, bad signatures and changed artifact bodies fail closed. See [`signed-remote-bundle.md`](signed-remote-bundle.md) for the publisher and staged-rotation contract.
...
Managed local lifecycle is now available, including source addition/removal through complete desired-state apply, explicit AgentWaker digest-key rotation, staged remote signing-key rotation and last-known-good hot reload. Broad customer rollout remains blocked on guided configuration/repair UI, hardware-backed publisher operation, live remote-origin/database/cross-platform failure-injection evidence, and operational recovery exercises.
