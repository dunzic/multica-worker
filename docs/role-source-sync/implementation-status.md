...
| RS-05 Secret and MCP synchronization | In progress, fork approach rejected | Plaintext-in-snapshot flaw rejected; source-neutral one-time envelope primitive binds transfer/workspace/source/role/snapshot/expiry with X25519 + HKDF-SHA256 + AES-256-GCM; at-rest helper now supports authenticated tenant/object context; AgentWaker daemon exporter reopens `.env`/MCP through bounded filesystem and rejects values/definitions changed since the approved snapshot | challenge/lease persistence, daemon protocol, encrypted versioned store, key IDs/rotation, apply-time consumption/runtime-only reveal, fail-closed audit and full exfiltration/replay suite |
